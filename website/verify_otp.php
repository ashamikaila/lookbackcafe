...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_otp'])) {
    ...
    // fetch latest unused OTP for the user
    $stmt = $conn->prepare("SELECT * FROM password_reset_otps WHERE user_id = ? AND used = 0 ORDER BY id DESC LIMIT 1");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();

    if (!$row) {
        $_SESSION['error'] = "No OTP found or it was already used. Request a new code.";
        header("Location: verify_otp.php");
        exit();
    }
    ...
    // verify OTP
    $stored_hash = $row['otp_hash']; // do NOT trim bcrypt hash
    ...
    // set session for reset password page
    $_SESSION['reset_user_id'] = $user_id;
    $_SESSION['otp_verified'] = true;
    $_SESSION['otp_verified_at'] = time();

    // keep reset_email in session for reset_password.php
    session_write_close();
...
inputs.forEach((input, index) => {
    ...
    input.addEventListener('keydown', e => {
        if (e.key === 'Backspace' && !e.target.value && index > 0) { inputs[index-1].focus(); inputs[index-1].value=''; }
    ...
    input.addEventListener('paste', e=>{
        ...
        for(let i=0;i<Math.min(pasted.length,inputs.length);i++) inputs[i].value=pasted[i];
...
document.getElementById('otpForm').addEventListener('submit', e=>{
    ...
    if(!allFilled){ e.preventDefault(); alert('Please enter all 4 digits'); inputs[0].focus();}
});
</script>
</body>
</html>
